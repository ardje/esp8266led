...
local function upload(id,fn)
	...
	local qfnn=string.format("%q",fn..".new")
	local fopen=string.format([[F=file.open(%s,"w") return F ~= nil and "open" or "error"]],qfnn)
	send(id,fopen)
	coroutine.yield()
	for l in encoder.lines("../flash/"..fn) do
	...
	send(id,fclose)
	coroutine.yield()
	local rename=string.format([[file.remove(%s) return file.rename(%s,%s) and "success" or "fail"]],qfn,qfnn,qfn)
	send(id,rename)
	coroutine.yield()
	print"file flashed closing down"
end
...
assert(aclient:subscribe("/esp/"..target.."/+",1,generic_cb))
--send("a0:20:a6:04:01:c8","node.restart()")
uploader()
-- We assume the upload takes at most 5s or the esp is down
aclient:message_loop(5)
